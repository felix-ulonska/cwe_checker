...
//!
//! Basically, the program searches for critical resources on an untrusted
//! search path that can be adjusted by an adversary. For example, see Nebula
//! Level 1 (<https://exploit-exercises.com/nebula/level01/>).
//!
...
//! startup. (Debian uses a modified bash which does not do this when invoked
//! as sh.)"
//!
//! See <https://cwe.mitre.org/data/definitions/426.html> for a detailed
//! description.
//!
//! ## How the check works
//!
//! We check whether a function that calls a privilege-changing function
//! (configurable in config.json) also calls system().
//!
//! ## False Positives
//!
...
//! ## False Negatives
//!
//! If the calls to the privilege-changing function and system() happen in
//! different functions, the calls will not be flagged as a CWE-hit.
//! This check only finds potential privilege escalation bugs, but other types
//! of bugs can also be triggered by untrusted search paths.
use super::prelude::*;
