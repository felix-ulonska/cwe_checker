...
pub fn build_memory_blocks(
    ...
    let global_analysis = infer_global_ptr(ssa_program, &project.runtime_memory_image);
    eprintln!("Finished Global Analysis within: {:02?}", start_time.elapsed());
    ...
    eprintln!("Finished Global Block Analysis within: {:02?}", start_time.elapsed());
    start_time = Instant::now();
    let stack_boundaries = build_stack_block(ssa_program);
    eprintln!("Finished Stack Block Analysis within: {:02?}", start_time.elapsed());
    start_time = Instant::now();
    let heap_analysis = build_heap_blocks(ssa_program, config);
    eprintln!("Finished Heap Analysis within: {:02?}", start_time.elapsed());
