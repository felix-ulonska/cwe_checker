use super::*;
use crate::ghidra_pcode::ExpressionOpcode::*;
use crate::ghidra_pcode::PcodeOpcode::Expression;
use crate::ghidra_pcode::PcodeOpcode::Jump;
use crate::variable;
use crate::{def, expr};
...
impl PcodeOperation {
    ...
    pub fn mock(pcode_op: &str) -> PcodeOperation {
        ...
        let input1 = components.next().map(|varnode| Varnode::mock(varnode));
...
pub fn mock_pcode_op_add(
    ...
    PcodeOperation {
        pcode_index: 1,
        pcode_mnemonic: Expression(INT_ADD),
        input0,
        input1,
        input2: None,
        output,
...
pub fn mock_pcode_op_cbranch(pcode_index: u64, input0: Varnode, input1: Varnode) -> PcodeOperation {
    PcodeOperation {
        pcode_index,
        pcode_mnemonic: Jump(CBRANCH),
        input0,
        input1: Some(input1),
        input2: None,
        output: None,
...
impl PcodeOperation {
    pub fn with_mnemonic(&self, mnemonic: PcodeOpcode) -> PcodeOperation {
        PcodeOperation {
    ...
    pub fn with_index(mut self, index: u64) -> PcodeOperation {
...
fn test_pcode_op_has_implicit_load() {
    let pcode_op = PcodeOperation::mock("register_RAX_8 INT_ADD ram_0x42_8 register_RAX_8");
    assert!(pcode_op.has_implicit_load());
    let pcode_op = PcodeOperation::mock("STORE const_0x1b1_8 ram_0x42_8 register_RAX_8");
    assert!(pcode_op.has_implicit_load());
    let pcode_op = PcodeOperation::mock("STORE const_0x1b1_8 register_RAX_8 ram_0x42_8");
    assert!(pcode_op.has_implicit_load());
    // No implicit loads
    let pcode_op = PcodeOperation::mock("STORE const_0x1b1_8 register_RAX_8 register_RAX_8");
    assert!(!pcode_op.has_implicit_load());
    let pcode_op = PcodeOperation::mock("ram_0x42_8 INT_ADD register_RAX_8 register_RAX_8");
    assert!(!pcode_op.has_implicit_load());
}
...
fn test_implicit_load_translation() {
    ...
    assert_eq!(
        instr.terms[0].create_explicit_ir_def_loads("0x1000"),
        vec![expected_load0, expected_load1]
    );
    assert_eq!(
        &instr.terms[0].input0,
        &Varnode {
    ...
    assert_eq!(
        instr.terms[0].input1.as_ref().unwrap(),
        &Varnode {
            address_space: "unique".to_string(),
    ...
    assert_eq!(
        instr.terms[0].input2.as_ref().unwrap(),
        &Varnode {
            address_space: "unique".to_string(),
...
#[test]
fn test_create_load() {
    let pcode_op = PcodeOperation::mock("register_RAX_8 LOAD space_id_8 const_0x0012345_8");
    assert_eq!(
        pcode_op.to_ir_def_load(&"0xFFFFFF".to_string()),
        Term {
...
fn test_create_load_no_source() {
    PcodeOperation::mock("register_RAX_8 LOAD space_id_8").to_ir_def_load(&"0x123".to_string());
}

#[test]
#[should_panic]
fn test_create_load_no_output() {
    PcodeOperation::mock("LOAD space_id_8 const_0x200_8").to_ir_def_load(&"0x123".to_string());
}

#[test]
#[should_panic]
fn test_create_load_target_not_var() {
    PcodeOperation::mock("const_0x4321_8 LOAD space_id_8 const_0x200_8")
        .to_ir_def_load(&"0x123".to_string());
}

#[test]
fn test_create_store() {
    let pcode_op = PcodeOperation::mock("STORE space_id_8 register_RAX_8 const_0x42_8");
    assert_eq!(
        pcode_op.to_ir_def_store(&"0x00ABCDEF".to_string()),
        Term {
...
fn test_create_store_not_store() {
    PcodeOperation::mock("STORE space_id_8 register_RAX_8").to_ir_def_store(&"0x123".to_string());
}

#[test]
#[should_panic]
fn test_create_store_from_ram() {
    PcodeOperation::mock("STORE space_id_8 const_4321_8 ram_0xFFFF01_8")
        .to_ir_def_store(&"0x123".to_string());
}

#[test]
fn test_create_subpiece() {
    let op = PcodeOperation::mock("register_EAX_4 SUBPIECE const_0xAABBCCDD_8 const_0x3_1");
    ...
    assert_eq!(op.to_ir_def_subpiece(&"0x1234".to_string()), expected);
}

#[test]
#[should_panic]
fn test_create_subpiece_with_non_constant() {
    PcodeOperation::mock("register_EAX_4 SUBPIECE const_0xABCDEF_8 register_RAX_8")
        .to_ir_def_subpiece(&"0x1234".to_string());
}

#[test]
fn test_create_unop() {
    let op = PcodeOperation::mock("register_RAX_8 INT_NEGATE register_RAX_8");
    let mut expected = def!["instr_0x1234_0: RAX:8 = -(RAX:8)"];
    expected.tid.address = "0x1234".to_string();
    assert_eq!(op.to_ir_def_unop(&"0x1234".to_string()), expected);
}

#[test]
#[should_panic]
fn test_create_unop_not_expression_type() {
    PcodeOperation::mock("register_RAX_8 CALL register_RAX_8")
        .to_ir_def_unop(&"0xFFFF".to_string());
}

#[test]
#[should_panic]
fn test_create_unop_not_unop_type() {
    PcodeOperation::mock("register_RAX_8 INT_ADD register_RAX_8")
        .to_ir_def_unop(&"0xFFFF".to_string());
}

#[test]
fn test_create_biop() {
    let op = PcodeOperation::mock("register_RAX_8 INT_ADD register_RAX_8 const_0xCAFE_8");
    let expected = Term {
        tid: Tid::mock("instr_0x1234_0"),
        term: def!["RAX:8 = RAX:8 + 0xCAFE:8"].term,
    };
    assert_eq!(op.to_ir_def_biop(&"0x1234".to_string()), expected)
}

#[test]
#[should_panic]
fn test_create_biop_not_biop_type() {
    PcodeOperation::mock("register_RAX_8 COPY register_RAX_8 const_0xCAFE_8")
        .to_ir_def_biop(&"0x1234".to_string());
}

#[test]
fn test_create_cast_op() {
    let op = PcodeOperation::mock("register_RDI_8 INT_ZEXT const_0x1_1");
    ...
    assert_eq!(op.to_ir_def_castop(&"0x4321".to_string()), expected);
}

#[test]
#[should_panic]
fn test_create_castop_not_castop_type() {
    PcodeOperation::mock("register_RAX_8 COPY const_0x1_1").to_ir_def_castop(&"0x1234".to_string());
}

#[test]
fn test_create_assign() {
    let op = PcodeOperation::mock("register_ZF_1 COPY const_0x1_1");
    let expected = Term {
        tid: Tid::mock("instr_0x1111_0"),
        term: def!["ZF:1 = 0x1:1"].term,
    };
    assert_eq!(expected, op.to_ir_def_assign(&"0x1111".to_string()))
}

#[test]
#[should_panic]
fn test_create_assign_not_copy_type() {
    PcodeOperation::mock("register_ZF_1 BOOL_AND register_RAX_8 const_0xCAFE_4")
        .to_ir_def_assign(&"0x1234".to_string());
}
...
fn test_wrap_in_assign_or_store() {
    ...
    assert_eq!(
        expected,
        op.wrap_in_ir_def_assign_or_store(&"0x1234".to_string(), expr.clone())
    );
    // test Store
    let op = PcodeOperation::mock("ram_0x1000_4 INT_ADD register_EAX_4 const_0xCAFE_4");
    expected.term = def!["Store at 0x1000:4 := EAX:4 + 0xCAFE:4"].term;
    assert_eq!(
        op.wrap_in_ir_def_assign_or_store(&"0x1234".to_string(), expr),
        expected
    )
}
...
fn test_wrap_in_assign_or_store_output_not_variable_nor_implicit_store() {
    PcodeOperation::mock("const_0xFFFF_4 INT_ADD register_EAX_4 const_0xCAFE_4")
        .wrap_in_ir_def_assign_or_store(&"0x1234".to_string(), expr!("0x1111:4"));
}
